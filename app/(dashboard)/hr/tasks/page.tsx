...
export default async function TasksPage({ searchParams }: { 
  ...
  // 1. Fetch Stats
  const totalTasksRes = await cachedQuery("SELECT COUNT(*) as count FROM employee_tasks WHERE company_id = ?", [companyId]);
  const pendingTasksRes = await cachedQuery("SELECT COUNT(*) as count FROM employee_tasks WHERE company_id = ? AND status = 'pending'", [companyId]);
  const progressTasksRes = await cachedQuery("SELECT COUNT(*) as count FROM employee_tasks WHERE company_id = ? AND status = 'in_progress'", [companyId]);
  const completedTasksRes = await cachedQuery("SELECT COUNT(*) as count FROM employee_tasks WHERE company_id = ? AND status = 'completed'", [companyId]);
  const overdueTasksRes = await cachedQuery("SELECT COUNT(*) as count FROM employee_tasks WHERE company_id = ? AND due_date < CURRENT_DATE AND status IN ('pending', 'in_progress')", [companyId]);
