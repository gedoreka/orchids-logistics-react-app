...
import {
  ...
  Loader2, CheckCircle, Mail, Send, ArrowRight
} from "lucide-react";
...
export default function PromissoryNotesClient() {
  ...
  return (
    <div className="min-h-screen pb-20" dir={isRtl ? "rtl" : "ltr"}>
      ...
      >
        <div className="h-2 w-full bg-gradient-to-r from-amber-500 via-orange-500 via-yellow-500 via-amber-500 to-orange-500" />
        <div className="p-6 md:p-10 space-y-6">
          ...
          >
            <div className="absolute -bottom-8 -right-8 w-40 h-40 bg-amber-500/10 rounded-full blur-3xl pointer-events-none" />
            ...
            <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-8 relative z-10">
              <div>
                <div className="flex items-center gap-2 text-xs font-bold text-slate-300 mb-3">
                  <a href="/dashboard" className="hover:text-amber-400 transition-colors">{isRtl ? 'لوحة التحكم' : 'Dashboard'}</a>
                  <ArrowRight size={13} className={`${isRtl ? 'rotate-180' : ''} text-slate-500`} />
                  <a href="/financial-vouchers" className="hover:text-amber-400 transition-colors">{isRtl ? 'السندات المالية' : 'Financial Vouchers'}</a>
                  <ArrowRight size={13} className={`${isRtl ? 'rotate-180' : ''} text-slate-500`} />
                  <span className="text-amber-400">{t('title')}</span>
                </div>
                <div className="flex items-center gap-5">
                  <div className="h-16 w-16 rounded-[1.25rem] bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center shadow-lg shadow-amber-500/25 flex-shrink-0">
                    <ScrollText size={32} strokeWidth={2.5} className="text-white" />
                  </div>
                  <div>
              ...
              <div className="flex flex-wrap gap-3 mt-4">
                <button
                  ...
                  className={cn(
                    ...
                    showForm ? "bg-white/10 text-white border border-white/20 hover:bg-white/20" : "bg-gradient-to-r from-amber-600 to-orange-600 text-white hover:from-amber-500 hover:to-orange-500 shadow-amber-500/25"
                  )}
                >
                ...
                >
                  <RefreshCw size={16} className={cn("text-amber-400", loading ? "animate-spin" : "")} />
                  {t('refreshData')}
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-6">
              <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-amber-500 to-orange-600 p-5 shadow-xl shadow-amber-500/25">
                <div className="p-2 bg-white/10 rounded-xl border border-white/10 w-fit mb-3"><ScrollText className="w-5 h-5 text-white" /></div>
                <p className="text-white/70 text-[10px] font-black uppercase tracking-widest">{t('totalCount')}</p>
                <p className="text-3xl font-black text-white mt-1">{notes.length}</p>
                <p className="text-white/50 text-[10px] font-black mt-0.5">{t('registeredNotes')}</p>
              ...
              <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-500 to-teal-600 p-5 shadow-xl shadow-emerald-500/25">
                <div className="p-2 bg-white/10 rounded-xl border border-white/10 w-fit mb-3"><Building2 className="w-5 h-5 text-white" /></div>
                <p className="text-white/70 text-[10px] font-black uppercase tracking-widest">{t('entity')}</p>
                <p className="text-lg font-black text-white truncate">{companyInfo?.name || "..."}</p>
