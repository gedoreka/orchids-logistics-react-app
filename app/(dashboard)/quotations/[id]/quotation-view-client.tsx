  ...
  try {
    ...
    try {
      if (invoiceDate && typeof invoiceDate === 'string') {
        safeDate = invoiceDate.split('T')[0];
      } else if (invoiceDate instanceof Date) {
        safeDate = invoiceDate.toISOString().split('T')[0];
      } else if (invoiceDate) {
        safeDate = new Date(invoiceDate).toISOString().split('T')[0];
      } else {
...
export function QuotationViewClient({ quotation, company, companyId }: QuotationViewClientProps) {
  ...
  const handleSendEmail = async () => {
    ...
    showNotification("loading", isRtl ? "جاري الإرسال" : "Sending", isRtl ? "جاري إعداد العرض وإرساله..." : "Preparing and sending quotation...");
    ...
    try {
      ...
      if (res.ok) {
        showNotification("success", isRtl ? "تم الإرسال" : "Sent Successfully", isRtl ? "تم إرسال العرض بنجاح كملف PDF" : "Quotation sent successfully as PDF");
