...
interface ReceiptVoucherViewClientProps {
  voucher: Voucher;
  company: Company;
  companyId: number;
}
...
export function ReceiptVoucherViewClient({ voucher, company, companyId }: ReceiptVoucherViewClientProps) {
  ...
  const handleDelete = async () => {
    if (!confirm(isRtl ? `هل أنت متأكد من حذف سند القبض رقم ${voucher.receipt_number}؟` : `Are you sure you want to delete voucher ${voucher.receipt_number}?`)) return;
    
    setDeleteLoading(true);
    showNotification("loading", isRtl ? "جاري الحذف" : "Deleting", isRtl ? "جاري حذف سند القبض..." : "Deleting receipt voucher...");
    
    try {
      const res = await fetch(`/api/receipt-vouchers/${voucher.id}?company_id=${companyId}`, {
        method: "DELETE"
      });
      
      if (res.ok) {
        showNotification("success", isRtl ? "تم الحذف بنجاح" : "Deleted Successfully", isRtl ? "تم حذف سند القبض بنجاح" : "Receipt voucher has been deleted successfully");
        setTimeout(() => {
          router.push("/receipt-vouchers");
          router.refresh();
        }, 1500);
      } else {
        showNotification("error", isRtl ? "فشل الحذف" : "Delete Failed", isRtl ? "فشل حذف سند القبض" : "Failed to delete receipt voucher");
      }
    } catch {
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `${isRtl ? "سند-قبض" : "Receipt-Voucher"}-${voucher.receipt_number}`,
  });

  const qrData = JSON.stringify({
    type: "Receipt Voucher",
    number: voucher.receipt_number,
    ...
    <div className="h-full flex flex-col bg-transparent" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="no-print p-4 md:p-6 pb-0 flex flex-wrap gap-2 justify-between items-center max-w-[1200px] mx-auto w-full">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-emerald-500 border border-gray-100">
            <Receipt size={20} />
          </div>
          <div>
            <h1 className="font-black text-white text-lg md:text-xl">
              {isRtl ? `سند قبض رقم ${voucher.receipt_number}` : `Receipt Voucher #${voucher.receipt_number}`}
            </h1>
            <p className="text-white/60 text-xs md:text-sm font-medium">{isRtl ? "تفاصيل سند القبض" : "Receipt voucher details"}</p>
          </div>
        </div>
        ...
        <div className="max-w-[210mm] mx-auto bg-white shadow-2xl rounded-3xl overflow-hidden print:rounded-none print:shadow-none print-content invoice-card">
          ...
          >
            <div className="flex flex-row justify-between items-center gap-4 relative z-10">
              ...
              <div className="text-center flex-1">
                <h1 className="text-3xl font-black mb-1 tracking-wider uppercase">{isRtl ? "سند قبض" : "Receipt Voucher"}</h1>
                <p className="text-white/60 text-[14px] uppercase font-light tracking-[0.2em]">{isRtl ? "إيصال استلام مالي" : "Financial Receipt"}</p>
            ...
            <div className="grid grid-cols-2 gap-6 mt-10 pt-6 border-t border-[#ffffff1a] relative z-10">
              <div className="text-center">
                ...
                <p className="font-black text-[15px] tracking-[0.1em]">{voucher.receipt_number}</p>
              </div>
              <div className="text-center border-s border-[#ffffff1a]">
                ...
                <p className="font-black text-[15px]">{voucher.receipt_date ? format(new Date(voucher.receipt_date), 'yyyy/MM/dd') : '-'}</p>
              </div>
            </div>
          ...
          <div className="p-8 md:p-12 space-y-10 flex-grow flex flex-col bg-white print-body">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 print-info-grid">
              ...
              <div className="relative group">
                <div className="absolute inset-0 bg-emerald-50 rounded-3xl rotate-1 group-hover:rotate-0 transition-transform duration-300" />
                <div className="relative rounded-3xl p-6 border border-emerald-100 bg-white shadow-sm flex flex-col h-full">
                  <div className="flex items-center gap-3 mb-5 pb-3 border-b border-gray-50">
                    <div className="h-10 w-10 rounded-xl bg-emerald-500 flex items-center justify-center text-white shadow-md">
                      <User size={20} />
                    </div>
                    <div>
                      <h3 className="font-black text-gray-900 text-sm">{isRtl ? "استلمنا من" : "Received From"}</h3>
                      <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Payer Details</p>
                    </div>
                  </div>
                  <div className="space-y-3.5 flex-1">
                    <DetailItem label={isRtl ? "الاسم" : "Name"} value={voucher.received_from} isRtl={isRtl} />
                    ...
                    <DetailItem label={isRtl ? "رقم المستند" : "Document No."} value={voucher.document_number || '-'} isRtl={isRtl} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-100 overflow-hidden shadow-sm bg-gray-50 p-6">
              ...
              <p className="text-gray-700 font-medium leading-relaxed">
                {voucher.description || (isRtl ? "لا يوجد بيان" : "No description")}
              </p>
              {voucher.notes && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-end">
              ...
              <div className="relative">
                <div className="absolute inset-0 bg-emerald-600 rounded-3xl translate-x-1.5 translate-y-1.5 opacity-5" />
                  ...
                  <div className="p-6 md:p-8 space-y-4">
                    ...
                    <div className="flex justify-between items-center text-amber-600 font-bold text-sm">
                      <span className="uppercase tracking-wider flex items-center gap-1.5">
                        <Percent size={14} />
                        {isRtl ? "الضريبة" : "VAT"} ({voucher.tax_rate}%)
                      </span>
                      <span>{Number(voucher.tax_value).toLocaleString('en-US', { minimumFractionDigits: 2 })} {currency}</span>
                    </div>
                    <div className="pt-4 border-t border-gray-100 mt-2">
                      <div className="flex justify-between items-center p-5 bg-gray-900 rounded-2xl text-white shadow-lg shadow-gray-200 print-grand-total">
                        ...
                        <div className="text-right">
                          <span className="block text-[10px] text-emerald-400 font-black mb-1 uppercase tracking-widest">Grand Total</span>
