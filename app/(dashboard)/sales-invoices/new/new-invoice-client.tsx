...
interface InvoiceItem {
  ...
  before_vat: number;
  vat_amount: number;
  calculation_mode: 'total' | 'quantity';
...
export function NewInvoiceClient({ customers, invoiceNumber, companyId, userName }: NewInvoiceClientProps) {
  ...
  const calculateItem = useCallback((item: InvoiceItem): InvoiceItem => {
    ...
    if (item.calculation_mode === 'quantity') {
      if (item.is_unit_price_inclusive) {
        totalWithVat = quantity * unitPrice;
        beforeVat = totalWithVat / 1.15;
        vatAmount = totalWithVat - beforeVat;
      } else {
        beforeVat = quantity * unitPrice;
        vatAmount = beforeVat * 0.15;
  ...
  const toggleMode = (index: number) => {
    setItems(prev => {
      const newItems = [...prev];
      newItems[index].calculation_mode = newItems[index].calculation_mode === 'total' ? 'quantity' : 'total';
      newItems[index] = calculateItem(newItems[index]);
      return newItems;
    });
  };

  const toggleInclusive = (index: number) => {
    setItems(prev => {
      const newItems = [...prev];
      newItems[index].is_unit_price_inclusive = !newItems[index].is_unit_price_inclusive;
      newItems[index] = calculateItem(newItems[index]);
      return newItems;
    });
  };
    ...
    try {
      const res = await fetch('/api/sales-invoices', {
        ...
        body: JSON.stringify({
          ...
          adjustments: adjustments.filter(a => a.title.trim()).map(adj => ({
            title: adj.title,
            type: adj.type,
            amount: adj.amount,
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="max-w-[90%] mx-auto px-4 pt-8"
      >
        <div className="bg-white rounded-[2.5rem] shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
          ...
          <div className="p-8 space-y-10">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  {selectedCustomer && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-6 bg-white rounded-2xl border border-gray-100 shadow-sm">
                      <div className="flex items-center gap-4">
                        <div className="h-10 w-10 rounded-xl bg-blue-50 flex items-center justify-center shadow-sm"><Building2 size={18} className="text-blue-500" /></div>
                        <div>
                          <span className="text-[10px] font-bold text-black uppercase block">{t("companyNameLabel")}</span>
                          <span className="text-sm font-black text-gray-800">{selectedCustomer.company_name || selectedCustomer.customer_name || selectedCustomer.name}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="h-10 w-10 rounded-xl bg-emerald-50 flex items-center justify-center shadow-sm"><CreditCard size={18} className="text-emerald-500" /></div>
                        <div>
                ...
                <table className="w-full min-w-[1000px]">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-100">
                      <th className="px-4 py-4 text-[10px] font-black text-black uppercase text-right w-12">{t("itemNo")}</th>
                      <th className="px-4 py-4 text-[10px] font-black text-black uppercase text-right">{t("serviceName")}</th>
                      <th className="px-4 py-4 text-[10px] font-black text-black uppercase text-center w-24">{t("quantity")}</th>
                      <th className="px-4 py-4 text-[10px] font-black text-black uppercase text-center w-40">{t("unitPrice")}</th>
                      <th className="px-4 py-4 text-[10px] font-black text-black uppercase text-center w-40">{t("totalInclusive")}</th>
                      <th className="px-4 py-4 text-[10px] font-black text-black uppercase text-center w-24">{t("tax")}</th>
                      <th className="px-4 py-4 text-[10px] font-black text-black uppercase text-center w-12">{t("delete")}</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={item.id} className="group hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-4 py-4">
                          <input
                            ...
                            className="w-full px-3 py-2 rounded-xl bg-white border border-gray-200 text-sm font-bold focus:border-emerald-500/30 outline-none"
                          />
                        </td>
                        <td className="px-4 py-4">
                          <input
                            ...
                            className="w-full px-2 py-2 rounded-xl bg-white border border-gray-200 text-sm font-black text-center"
                          />
                        </td>
                        <td className="px-4 py-4">
                          <div className="flex flex-col gap-2">
                            <div className="relative">
                              <input
                                ...
                                readOnly={item.calculation_mode === 'total'}
                                onChange={(e) => handleItemChange(index, 'unit_price', parseFloat(e.target.value) || 0)}
                                className={cn(
                                  "w-full px-2 py-2 rounded-xl border text-sm font-black text-center outline-none",
                                  item.calculation_mode === 'total' ? "bg-gray-50 border-gray-100 text-gray-400 cursor-not-allowed" : "bg-white border-blue-200 text-blue-600 focus:border-blue-400"
                                )}
                              />
                              <button
                                type="button"
                                onClick={() => toggleMode(index)}
                                title={t("calcByQuantity")}
                                className={cn(
                                  "absolute -right-8 top-1/2 -translate-y-1/2 p-1.5 rounded-lg transition-all",
                                  item.calculation_mode === 'quantity' ? "bg-blue-500 text-white shadow-lg scale-110" : "bg-gray-100 text-gray-400 hover:bg-gray-200"
                                )}
                              >
                                <Scale size={14} />
                              </button>
                            ...
                            {item.calculation_mode === 'quantity' && (
                              <button
                                type="button"
                                onClick={() => toggleInclusive(index)}
                                className={cn(
                                  "text-[9px] font-black py-1 px-2 rounded-lg border transition-all",
                                  item.is_unit_price_inclusive ? "bg-emerald-50 border-emerald-200 text-emerald-600" : "bg-amber-50 border-amber-200 text-amber-600"
                                )}
                              >
                                {item.is_unit_price_inclusive ? t("inclusiveTax") : tc("exclusive")}
                              </button>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-4">
                          <div className="relative">
                            <input
                              ...
                              readOnly={item.calculation_mode === 'quantity'}
                              onChange={(e) => handleItemChange(index, 'total_with_vat', parseFloat(e.target.value) || 0)}
                              className={cn(
                                "w-full px-2 py-2 rounded-xl border text-sm font-black text-center outline-none",
                                item.calculation_mode === 'quantity' ? "bg-gray-50 border-gray-100 text-gray-400 cursor-not-allowed" : "bg-emerald-50 border-emerald-100 text-emerald-600 focus:border-emerald-400"
                              )}
                            />
                            {item.calculation_mode === 'quantity' && (
                              <div className="absolute -right-8 top-1/2 -translate-y-1/2 p-1.5 rounded-lg bg-gray-100 text-gray-400 opacity-50">
                                <DollarSign size={14} />
                              </div>
                            )}
                          </div>
                ...
                <div className="space-y-4">
                  {adjustments.length === 0 ? (
                    <div className="text-center py-12 bg-white rounded-[2.5rem] border-2 border-dashed border-gray-200">
                      <Percent size={32} className="text-gray-300 mx-auto mb-2" />
                      <p className="text-black font-black text-sm">{t("noAdjustments")}</p>
                    </div>
                  ) : (
                    adjustments.map((adj, index) => (
                      <div key={adj.id} className="p-6 bg-white rounded-[2rem] border border-gray-100 shadow-sm space-y-6">
                          ...
                          <div className="flex items-center gap-6">
                            <label className="flex items-center gap-3 cursor-pointer group">
                              <input
                                ...
                                className="peer h-5 w-5 rounded-lg border-gray-300 text-amber-500 focus:ring-amber-500"
                              />
                  ...
                  <div className="space-y-6 relative z-10">
                    ...
                    <div className="pt-2">
                      <div className="p-6 bg-emerald-500/10 rounded-[1.5rem] border border-emerald-500/20 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                          <CircleDollarSign size={24} className="text-emerald-400" />
            ...
            <div className="pt-10 mt-10 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-8">
              <Link href="/sales-invoices">
                <motion.button whileHover={{ x: -5 }} className="flex items-center gap-2 text-black hover:opacity-70 transition-all font-black text-sm uppercase tracking-widest">
              ...
              <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                <motion.button
                  ...
                  disabled={loading}
                  className="flex-1 sm:flex-none flex items-center justify-center gap-3 px-10 py-4 rounded-2xl bg-slate-800 text-white hover:bg-slate-900 font-black transition-all text-sm shadow-xl"
                >
                ...
                <motion.button
                  ...
                  disabled={loading}
                  className="flex-1 sm:flex-none flex items-center justify-center gap-3 px-12 py-4 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-black transition-all text-sm shadow-xl"
                >
