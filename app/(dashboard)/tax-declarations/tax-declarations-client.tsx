...
function TaxDeclarationsContent({ companyId }: { companyId: number }) {
  ...
  }, [companyId, statusFilter, yearFilter]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 font-black text-sm">جاري تحميل الإقرارات الضريبية...</p>
        </motion.div>
      </div>
    );
  }
              ...
              >
                ...
                <div className="p-8 text-center space-y-6">
                  ...
                  <div className="bg-amber-950/30 border border-amber-900/50 rounded-xl p-3">
                    <p className="text-amber-400 text-xs font-bold text-center">لا يمكن التراجع عن هذا الإجراء بعد التنفيذ</p>
                  </div>
        ...
        {showViewModal && selectedDeclaration && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              transition={{ type: "spring", damping: 25, stiffness: 300 }}
              className="bg-slate-900 rounded-[2rem] shadow-[0_0_100px_rgba(99,102,241,0.2)] w-full max-w-2xl max-h-[90vh] overflow-hidden border-2 border-indigo-500/20"
              onClick={e => e.stopPropagation()}>

              {/* Header */}
              <div className="relative bg-gradient-to-r from-indigo-600 via-violet-600 to-indigo-700 p-6 text-white flex justify-between items-center overflow-hidden">
                <div className="absolute inset-0 opacity-10">
                  <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent" />
                </div>
                <h3 className="text-xl font-black flex items-center gap-3 relative z-10">
                  <div className="p-2 bg-white/20 rounded-xl"><Eye size={20} /></div>
                  تفاصيل الإقرار الضريبي
                </h3>
                <div className="flex items-center gap-3 relative z-10">
                  <span className={cn("px-3 py-1 rounded-lg text-xs font-black border", getStatusBadge(selectedDeclaration.status))}>
              ...
              <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)] space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">الفترة الضريبية</p>
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">فترة التغطية</p>
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-gradient-to-r from-indigo-600/20 to-violet-600/20 border border-indigo-500/30 rounded-2xl p-5 flex justify-between items-center mt-4">
                ...
                <div className="flex gap-3 pt-4 border-t border-white/10">
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    onClick={() => setShowViewModal(false)}
                    className="flex-1 px-6 py-3.5 bg-white/5 hover:bg-white/10 text-slate-400 rounded-xl font-black text-sm border border-white/10 transition-all">
                    إغلاق
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    onClick={() => handlePrint(selectedDeclaration)}
                    className="flex-1 flex items-center justify-center gap-2 px-6 py-3.5 bg-gradient-to-r from-indigo-500 to-violet-600 text-white rounded-xl font-black text-sm shadow-xl shadow-indigo-500/20 transition-all">
        ...
        {showEditModal && selectedDeclaration && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              exit={{ scale: 0.9, opacity: 0, y: 20 }}
              transition={{ type: "spring", damping: 25, stiffness: 300 }}
              className="bg-slate-900 rounded-[2rem] shadow-[0_0_100px_rgba(245,158,11,0.2)] w-full max-w-md overflow-hidden border-2 border-amber-500/20"
              onClick={e => e.stopPropagation()}>

              <div className="relative bg-gradient-to-r from-amber-500 via-orange-600 to-amber-600 p-6 text-white flex justify-between items-center overflow-hidden">
                <h3 className="text-xl font-black flex items-center gap-3 relative z-10">
                  <div className="p-2 bg-white/20 rounded-xl"><Edit size={20} /></div>
                  تعديل حالة الإقرار
                </h3>
              ...
              <div className="p-6 space-y-3">
                <p className="text-slate-400 text-sm font-bold mb-4">اختر الحالة الجديدة للإقرار الضريبي</p>
                {[
                  { value: "draft", label: "مسودة", icon: Clock, color: "amber", desc: "الإقرار قيد المراجعة" },
                  { value: "submitted", label: "تم التقديم", icon: Send, color: "emerald", desc: "تم تقديم الإقرار رسمياً" },
                  { value: "completed", label: "مكتمل ومسدد", icon: CheckCircle2, color: "blue", desc: "تم السداد والاكتمال" },
                ].map(s => (
                  <motion.button
                    key={s.value}
                  ...
                  >
                    <div className={cn(
                      "p-2.5 rounded-xl",
                      selectedDeclaration.status === s.value ? `bg-${s.color}-500/30` : "bg-white/10"
                    )}>
                      <s.icon size={18} />
                    </div>
                    <div>
        ...
        {showCreateModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              exit={{ scale: 0.9, opacity: 0, y: 20 }}
              transition={{ type: "spring", damping: 25, stiffness: 300 }}
              className="bg-slate-900 rounded-[2rem] shadow-[0_0_100px_rgba(99,102,241,0.2)] w-full max-w-4xl max-h-[90vh] overflow-hidden border-2 border-indigo-500/20"
              onClick={e => e.stopPropagation()}>
              ...
              <div className="relative bg-gradient-to-r from-indigo-600 via-violet-600 to-indigo-700 p-6 text-white flex justify-between items-center overflow-hidden">
                ...
                <h3 className="text-xl font-black flex items-center gap-3 relative z-10">
                  <div className="p-2 bg-white/20 rounded-xl"><PlusCircle size={20} /></div>
              ...
              {/* Stepper */}
              <div className="px-8 py-4 bg-slate-800/50 border-b border-white/5">
                  ...
                  {[
                    ...
                    <div key={s.n} className="relative z-10 flex flex-col items-center gap-2">
                      <div className={cn(
                        ...
                        {createStep > s.n ? <CheckCircle2 size={18} /> : s.n}
                      </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {createStep === 1 && (
                    <motion.div key="step1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}
                      ...
                      <div className="text-center mb-6">
                        <h4 className="text-white font-black text-lg">تحديد الفترة الضريبية</h4>
                        <p className="text-slate-500 text-sm font-bold mt-1">اختر السنة والربع السنوي المراد إصدار الإقرار له</p>
                      </div>
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            {["2024", "2025", "2026"].map(y => (
                              <motion.button key={y} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                                ...
                                className={cn(
                                  "p-3.5 rounded-xl font-black text-sm transition-all border",
                                  period.year === y
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              ...
                              <motion.button key={q.v} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                                ...
                                className={cn(
                                  "p-3.5 rounded-xl font-black text-sm transition-all border text-right",
                                  period.quarter === q.v
                                    ? "bg-gradient-to-r from-indigo-500 to-violet-600 text-white border-indigo-400 shadow-lg shadow-indigo-500/30"
                                    : "bg-white/5 text-slate-400 border-white/10 hover:bg-white/10"
                                )}>
                      ...
                      <motion.button
                        ...
                        disabled={collecting}
                        className="w-full flex items-center justify-center gap-3 px-8 py-4 bg-gradient-to-r from-indigo-500 to-violet-600 text-white rounded-2xl font-black text-base shadow-xl shadow-indigo-500/25 disabled:opacity-50 transition-all"
                      >
                        {collecting ? (
                  ...
                  {createStep === 2 && (
                    <motion.div key="step2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-2xl p-6 space-y-4">
                          <div className="flex items-center gap-3">
                            <div className="p-2 bg-emerald-500/20 rounded-xl"><TrendingUp size={20} className="text-emerald-400" /></div>
                            <div>
                        ...
                        {/* Input Tax Card */}
                        <div className="bg-gradient-to-br from-red-500/10 to-rose-500/10 border border-red-500/20 rounded-2xl p-6 space-y-4">
                          <div className="flex items-center gap-3">
                      ...
                      {/* Net Tax */}
                      <div className="bg-gradient-to-r from-indigo-600/20 to-violet-600/20 border border-indigo-500/30 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
                        <div>
                      ...
                      <div className="flex gap-3 pt-4 border-t border-white/10">
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                          ...
                          className="flex items-center gap-2 px-6 py-3 bg-white/5 hover:bg-white/10 text-slate-400 rounded-xl font-black text-sm border border-white/10 transition-all">
                          <ChevronRight size={16} />تعديل الفترة
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                          onClick={() => setCreateStep(3)}
                          className="flex-1 flex items-center justify-center gap-2 px-8 py-3 bg-gradient-to-r from-indigo-500 to-violet-600 text-white rounded-xl font-black text-sm shadow-xl shadow-indigo-500/20 transition-all">
                  ...
                  {createStep === 3 && (
                    <motion.div key="step3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}
                      ...
                      <div className="text-center mb-4">
                        <h4 className="text-white font-black text-lg">التأكيد النهائي والتقديم</h4>
                        <p className="text-slate-500 text-sm font-bold mt-1">يرجى مراجعة كافة المبالغ قبل الحفظ النهائي</p>
                      </div>

                      {/* Summary */}
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {[
                          { label: "السنة", value: period.year },
                          { label: "الربع", value: `الربع ${period.quarter}` },
                          { label: "ضريبة المخرجات", value: formatNum(collectData.total_output_tax), color: "text-emerald-400" },
                          { label: "ضريبة المدخلات", value: formatNum(collectData.total_input_tax), color: "text-red-400" },
                        ].map((item, i) => (
                      ...
                      <div className="bg-gradient-to-r from-indigo-600/20 to-violet-600/20 border border-indigo-500/30 rounded-2xl p-5 text-center">
                        <p className="text-indigo-400 text-xs font-black uppercase tracking-widest mb-1">صافي الضريبة المستحقة</p>
                      ...
                      <div className="flex gap-3 pt-4 border-t border-white/10">
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                          ...
                          className="flex items-center gap-2 px-4 py-3 bg-white/5 hover:bg-white/10 text-slate-400 rounded-xl font-black text-sm border border-white/10 transition-all">
                          <ChevronRight size={16} />رجوع
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                          onClick={() => handleSaveDeclaration("draft")}
                          disabled={saving}
                          className="flex items-center gap-2 px-6 py-3 bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 rounded-xl font-black text-sm border border-amber-500/20 disabled:opacity-30 transition-all">
                          <Clock size={16} />حفظ كمسودة
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                          onClick={() => handleSaveDeclaration("submitted")}
                          disabled={saving}
                          className="flex-1 flex items-center justify-center gap-2 px-8 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl font-black text-sm shadow-xl shadow-emerald-500/20 disabled:opacity-30 transition-all">
        ...
        <div className="bg-slate-900 rounded-3xl border border-white/5 shadow-2xl overflow-hidden flex flex-col">
          ...
          <div className="p-8 space-y-8 bg-slate-900 border-b border-white/5">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-4">
                <motion.button
                  ...
                  className="flex items-center gap-2.5 px-6 py-3.5 rounded-2xl bg-gradient-to-r from-indigo-600 to-violet-600 text-white hover:from-indigo-700 hover:to-violet-700 transition-all font-black text-sm shadow-xl shadow-indigo-500/25"
                >
                  <PlusCircle size={20} />
                  إنشاء إقرار جديد
                </motion.button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              {[
                ...
                { label: "إقرارات مقدمة", value: String(stats.submittedCount), icon: CheckCircle2, gradient: "from-blue-500 to-indigo-600", shadow: "shadow-blue-500/20", isCount: true },
                { label: "إقرارات معلقة", value: String(stats.draftCount), icon: Clock, gradient: "from-amber-500 to-orange-600", shadow: "shadow-amber-500/20", isCount: true },
              ].map((card, i) => (
          ...
          <div className="bg-slate-900/80 px-8 py-5 border-b border-white/5 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select value={yearFilter} onChange={e => setYearFilter(e.target.value)}
                className="py-2.5 px-3 bg-white/5 border border-white/10 rounded-xl text-white text-xs font-bold focus:border-indigo-500/50 outline-none transition-all appearance-none cursor-pointer [color-scheme:dark]">
          ...
          <div className="bg-slate-900 px-8 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-white font-black tracking-tight">سجل الإقرارات الضريبية</h3>
                <p className="text-slate-400 text-xs font-bold tracking-wide uppercase">{filteredDeclarations.length} إقرار</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            <div className="space-y-3">
              {filteredDeclarations.length > 0 ? (
                filteredDeclarations.map((d, idx) => (
                  <motion.div key={d.id} variants={itemVariants}
                    ...
                    <div className="p-5 flex justify-between items-center">
                      <div className="flex items-center gap-4 flex-wrap">
                        {/* Period Badge */}
                        <div className="px-4 py-2 rounded-xl font-black text-sm border bg-gradient-to-r from-indigo-500/10 to-violet-500/10 text-indigo-400 border-indigo-500/20">
                        ...
                        <span className={cn("px-3 py-1 rounded-lg text-[10px] font-black border", getStatusBadge(d.status))}>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex gap-2">
                          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                            ...
                            className="p-2 bg-indigo-500/20 text-indigo-400 rounded-xl hover:bg-indigo-500/30 transition-colors border border-indigo-500/20"
                            title="عرض التفاصيل">
                            <Eye size={16} />
                          </motion.button>
                          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                          ...
                          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                            ...
                            className="p-2 bg-blue-500/20 text-blue-400 rounded-xl hover:bg-blue-500/30 transition-colors border border-blue-500/20"
                            title="طباعة الإقرار">
                            <Printer size={16} />
                          </motion.button>
                          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                ...
                <div className="text-center py-20 rounded-3xl border-2 border-dashed border-white/10 bg-white/5">
                  ...
                  <h4 className="text-lg font-black text-slate-500">لا يوجد إقرارات ضريبية</h4>
                  <p className="text-slate-600 text-sm">ابدأ بإنشاء إقرار ضريبي جديد للفترة الحالية</p>
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => { resetCreateForm(); setShowCreateModal(true); }}
                    className="mt-6 px-6 py-3 bg-gradient-to-r from-indigo-500 to-violet-600 text-white rounded-xl font-black text-sm shadow-lg shadow-indigo-500/20"
                  >
