...
export async function POST(
  ...
  try {
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <style>
          ...
          .total { background: #fef2f2; padding: 20px; border-radius: 12px; margin-top: 20px; text-align: center; }
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>سند صرف</h1>
            <p style="margin: 10px 0 0; opacity: 0.8;">Payment Voucher</p>
          </div>
          <div class="content">
            <div class="info-row">
            ...
            <div class="info-row">
              <span class="label">صرفنا إلى:</span>
            ...
            <div class="info-row">
              ...
              <span class="value">${voucher.payment_purpose || '-'}</span>
            </div>
            <div class="total">
              <p style="margin: 0 0 10px; color: #666;">المبلغ الإجمالي</p>
              <div class="total-amount">${Number(voucher.total_amount).toLocaleString('en-US', { minimumFractionDigits: 2 })} ${voucher.currency}</div>
            </div>
          </div>
          <div class="footer">
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: `سند صرف رقم ${voucher.voucher_number} - ${company.name || 'الشركة'}`,
      html: htmlContent,
    });
