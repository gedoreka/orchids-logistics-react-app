...
import { sha256HashBase64, signHash, sha256Hash, getPublicKeyRawBytes, extractCertificateSignature } from "./crypto";
                  ...
                  <xades:SignedSignatureProperties>
                    ...
                    <xades:SigningCertificate>
                      <xades:Cert>
                        ...
                        <xades:IssuerSerial>
                          <ds:X509IssuerName>CN=ZATCA-SubCA-1,DC=zatca,O=ZATCA,C=SA</ds:X509IssuerName>
...
export function buildSignedXml(data: ZatcaInvoiceData, signing: SigningInfo): {
  ...
  // Step 2: Get hash input (remove extensions + QR, canonicalize)
  ...
  // Step 3: Sign the invoice hash
  const signatureValue = signHash(signing.privateKey, invoiceHashHex);

  // Step 4: Compute certificate digest
  const certDigest = sha256HashBase64(Buffer.from(signing.certificate, "base64").toString("binary"));
  ...
  // Step 6: Insert signature into XML
  let signedXml = unsignedXml.replace(
    "<!-- SIGNATURE_PLACEHOLDER -->",
    signatureBlock
  );

  // Step 7: Build QR code
