...
export function AccountsClient({ initialAccounts, companyId }: AccountsClientProps) {
  ...
  const accountTypeConfig: Record<string, { color: string; bgColor: string; borderColor: string; icon: any; label: string }> = {
    ...
    "اصل": { color: "text-blue-700", bgColor: "bg-blue-50", borderColor: "border-blue-200", icon: Building2, label: t("types.asset") },
    "التزام": { color: "text-amber-700", bgColor: "bg-amber-50", borderColor: "border-amber-200", icon: Receipt, label: t("types.liability") },
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center justify-center text-center space-y-6">
            ...
            <div className="space-y-2">
              <h1 className="text-3xl font-black tracking-tight bg-gradient-to-r from-white via-blue-100 to-white bg-clip-text text-transparent">{t("title")}</h1>
        ...
        >
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            <div className="relative flex-1 md:flex-none">
              <Filter className={cn("absolute top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4", isRtl ? "right-4" : "left-4")} />
              <select
                value={filterType}
                onChange={(e) => setFilterType(e.target.value)}
                className={cn(
                  "w-full md:w-56 bg-slate-50/50 border border-slate-200 rounded-2xl py-4 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all appearance-none text-sm font-black text-slate-700 shadow-sm cursor-pointer",
                  isRtl ? "pr-11 pl-6" : "pl-11 pr-6"
                )}
              >
                <option value="all">{t("allTypes")}</option>
                <option value="مصروف">{t("expenses")}</option>
                <option value="دخل">{t("income")}</option>
                <option value="اصل">{t("assets")}</option>
                <option value="التزام">{t("liabilities")}</option>
                <option value="حقوق ملكية">{t("equity")}</option>
              </select>
              <ChevronDown className={cn("absolute top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4 pointer-events-none", isRtl ? "left-4" : "right-4")} />
            </div>
            <button
        ...
        >
          ...
          <div className="bg-slate-50/50 px-8 py-6 border-t border-slate-100 flex items-center justify-between">
            <p className="text-sm text-slate-500 font-bold">
              {t("view")} <span className="text-slate-900 font-black px-1">{filteredAccounts.length}</span> {t("outOf")} <span className="text-slate-900 font-black px-1">{accounts.length}</span> {t("accountsCount")}
            </p>
          </div>
        </motion.div>
            ...
            <div className="fixed inset-0 z-[150] flex items-center justify-center p-4" dir={isRtl ? "rtl" : "ltr"}>
              ...
              >
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                  <div className="space-y-2">
                    <label className={cn("text-xs font-black text-slate-500 uppercase tracking-wider", isRtl ? "mr-1" : "ml-1")}>{t("accountCode")}</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={cn(
                          "w-full bg-slate-50 border border-slate-200 rounded-xl py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all text-sm font-bold",
                  ...
                  <div className="space-y-2">
                    <label className={cn("text-xs font-black text-slate-500 uppercase tracking-wider", isRtl ? "mr-1" : "ml-1")}>{t("accountName")}</label>
                    <div className="relative">
                      <FileText className={cn("absolute top-1/2 -translate-y-1/2 text-slate-300 w-4 h-4", isRtl ? "right-4" : "left-4")} />
                      <input
                        type="text"
                        required
                  ...
                  <div className="space-y-2">
                    <label className={cn("text-xs font-black text-slate-500 uppercase tracking-wider", isRtl ? "mr-1" : "ml-1")}>{t("accountType")}</label>
                  ...
                  <div className="flex gap-3 pt-4">
                    <button
                      type="submit"
                      disabled={isLoading}
                      className={cn("flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold transition-all flex items-center justify-center disabled:opacity-50 shadow-lg shadow-blue-200", isRtl ? "space-x-2 space-x-reverse" : "space-x-2")}
                    >
                      {isLoading ? (
