  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.error || "فشل تحديث العميل");
      }
    } catch (error) {
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[#2c3e50] to-[#34495e] rounded-[2rem] p-8 text-white shadow-2xl">
        <div className="relative z-10 space-y-4">
          <div className="flex flex-col items-center text-center space-y-4">
            <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center text-white shadow-lg shadow-amber-500/20">
              <Edit size={32} />
            </div>
            <div>
              <h1 className="text-3xl font-black tracking-tight">تعديل بيانات العميل</h1>
              <p className="text-white/60 text-sm mt-2">{customer.customer_name || customer.company_name}</p>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-3">
            ...
            <Link href={`/customers/${customer.id}`}>
              <button className="flex items-center gap-2 px-5 py-3 rounded-xl bg-blue-500 text-white font-bold text-sm hover:bg-blue-600 transition-all">
                <Eye size={18} />
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-center gap-4 pt-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex items-center gap-2 px-8 py-4 rounded-xl bg-gradient-to-br from-amber-500 to-amber-600 text-white font-bold text-sm hover:shadow-lg hover:shadow-amber-500/20 transition-all disabled:opacity-50"
          >
            {loading ? (
