      ...
      >
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 text-white flex items-center justify-between">
          <div className="flex items-center space-x-3 space-x-reverse">
            ...
            <div>
              <h2 className="text-2xl font-bold tracking-tight">إدارة أنواع المصروفات المخصصة</h2>
              <p className="text-blue-100 text-sm opacity-90">أضف وتعديل الأنواع الفرعية الخاصة بك فقط</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-blue-50 p-4 rounded-2xl text-center border border-blue-100">
              <p className="text-2xl font-bold text-blue-600">{subtypes.length}</p>
              <p className="text-sm text-blue-700 font-medium">إجمالي الأنواع</p>
            </div>
            <div className="bg-green-50 p-4 rounded-2xl text-center border border-green-100">
              <p className="text-2xl font-bold text-green-600">{subtypes.filter(s => s.is_custom).length}</p>
              <p className="text-sm text-green-700 font-medium">أنواع مخصصة لك</p>
            </div>
            <div className="bg-amber-50 p-4 rounded-2xl text-center border border-amber-100">
              <p className="text-2xl font-bold text-amber-600">{subtypes.filter(s => !s.is_custom).length}</p>
              <p className="text-sm text-amber-700 font-medium">أنواع عامة</p>
            </div>
          </div>

          {/* Add Form */}
          <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
            <div className="flex items-center space-x-2 space-x-reverse mb-4 text-slate-800">
              <Plus className="w-5 h-5 text-blue-600" />
              <h3 className="font-bold">إضافة نوع مخصص جديد</h3>
            </div>
            <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <button 
                type="submit"
                disabled={submitting}
                className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm transition-all flex items-center justify-center space-x-2 space-x-reverse"
              >
            ...
            <p className="text-xs text-slate-500 mt-3 flex items-center gap-1">
              ...
              ملاحظة: الأنواع التي تضيفها هنا ستكون خاصة بك فقط ويمكنك حذفها لاحقاً.
            </p>
          </div>
          ...
          <div className="space-y-6">
            {Object.entries(groupedSubtypes).map(([mainType, items]) => (
              <div key={mainType} className="space-y-3">
                <div className="flex items-center space-x-2 space-x-reverse px-2 border-r-4 border-blue-500">
                  <h4 className="font-bold text-slate-900">{mainTypes[mainType as keyof typeof mainTypes] || mainType}</h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {items.map((item) => (
                    <div 
                      key={item.id} 
                      className={`group relative p-4 rounded-xl border transition-all flex items-center justify-between ${item.is_custom ? 'bg-white border-slate-200 hover:border-blue-300' : 'bg-slate-50 border-slate-100 opacity-80'}`}
                    >
                      <div className="flex items-center space-x-3 space-x-reverse">
                        <div className={`w-2 h-2 rounded-full ${item.is_custom ? 'bg-blue-400' : 'bg-slate-400'}`} />
                        <span className={`text-sm font-medium ${item.is_custom ? 'text-slate-900' : 'text-slate-600'}`}>{item.subtype_name}</span>
                        {!item.is_custom && <span className="text-[10px] bg-slate-200 text-slate-500 px-1.5 py-0.5 rounded uppercase font-bold">عام</span>}
                      </div>
