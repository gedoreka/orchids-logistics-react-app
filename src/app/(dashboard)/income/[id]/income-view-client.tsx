...
interface IncomeViewClientProps {
  income: Income;
  company: Company;
  companyId: number;
}
...
export function IncomeViewClient({ income, company, companyId }: IncomeViewClientProps) {
  ...
  const handleDelete = async () => {
    if (!confirm(isRtl ? `هل أنت متأكد من حذف سند الإيراد رقم ${income.operation_number}؟` : `Are you sure you want to delete income ${income.operation_number}?`)) return;
    
    setDeleteLoading(true);
    showNotification("loading", isRtl ? "جاري الحذف" : "Deleting", isRtl ? "جاري حذف سند الإيراد..." : "Deleting income record...");
    
    try {
      const res = await fetch(`/api/income/${income.id}?company_id=${companyId}`, {
        method: "DELETE"
      });
      
      if (res.ok) {
        showNotification("success", isRtl ? "تم الحذف بنجاح" : "Deleted Successfully", isRtl ? "تم حذف سند الإيراد بنجاح" : "Income record has been deleted successfully");
        setTimeout(() => {
          router.push("/income/new");
          router.refresh();
        }, 1500);
      } else {
        showNotification("error", isRtl ? "فشل الحذف" : "Delete Failed", isRtl ? "فشل حذف سند الإيراد" : "Failed to delete income record");
      }
    } catch {
  ...
  const handleSendEmail = async () => {
    ...
    showNotification("loading", isRtl ? "جاري الإرسال" : "Sending", isRtl ? "جاري إرسال السند عبر البريد..." : "Sending income via email...");
    
    try {
      const res = await fetch(`/api/income/${income.id}/send-email`, {
        method: "POST",
      ...
      if (res.ok) {
        showNotification("success", isRtl ? "تم الإرسال" : "Sent Successfully", isRtl ? "تم إرسال السند بنجاح" : "Income sent successfully");
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `${isRtl ? "سند-إيراد" : "Income-Voucher"}-${income.operation_number}`,
  });

  const qrData = JSON.stringify({
    type: "Income Voucher",
    number: income.operation_number,
    ...
    <div className="h-full flex flex-col bg-transparent" dir={isRtl ? "rtl" : "ltr"}>
            ...
            >
              <div className="bg-white rounded-[2.5rem] p-10 shadow-2xl border border-gray-100">
                <div className="flex items-center gap-4 mb-8">
                  <div className="h-16 w-16 rounded-2xl bg-cyan-500/10 text-cyan-500 flex items-center justify-center">
                    <Mail size={32} />
                  </div>
                  <div>
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full px-6 py-4 rounded-2xl bg-gray-50 text-black border border-gray-100 focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 outline-none transition-all font-bold"
                    />
                  </div>
                  ...
                  <div className="flex gap-4 pt-4">
                    ...
                    <button
                      ...
                      className="flex-1 py-4 rounded-2xl bg-cyan-600 text-white font-black hover:bg-cyan-700 transition-all shadow-xl shadow-cyan-200 disabled:opacity-50 flex items-center justify-center gap-2"
                    >
      ...
      <div className="no-print p-4 md:p-6 pb-0 flex flex-wrap gap-2 justify-between items-center max-w-[1200px] mx-auto w-full">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-cyan-500 border border-gray-100">
            <BadgeDollarSign size={20} />
          </div>
          <div>
            <h1 className="font-black text-white text-lg md:text-xl">
              {isRtl ? `سند إيراد رقم ${income.operation_number}` : `Income Voucher #${income.operation_number}`}
            </h1>
            <p className="text-white/60 text-xs md:text-sm font-medium">{isRtl ? "تفاصيل سند الإيراد" : "Income voucher details"}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white text-cyan-600 font-bold text-xs md:text-sm hover:bg-cyan-50 transition-all border border-cyan-100 shadow-sm"
          >
            <Mail size={16} />
          ...
          <button 
            onClick={() => handlePrint()}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-cyan-500 text-white font-bold text-xs md:text-sm hover:bg-cyan-600 transition-all shadow-md"
          >
            <Printer size={16} />
          ...
          >
            <div className="flex flex-row justify-between items-center gap-4 relative z-10">
              ...
              <div className="text-center flex-1">
                <h1 className="text-3xl font-black mb-1 tracking-wider uppercase">{isRtl ? "سند إيراد" : "Income Voucher"}</h1>
                <p className="text-white/60 text-[14px] uppercase font-light tracking-[0.2em]">{isRtl ? "إيصال إيراد مالي" : "Financial Income"}</p>
              ...
              <div className="flex flex-col items-center gap-2 p-4 rounded-2xl border border-[#ffffff1a] min-w-[140px]" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }}>
                <Truck size={32} className="text-teal-300" />
            ...
            <div className="grid grid-cols-2 gap-6 mt-10 pt-6 border-t border-[#ffffff1a] relative z-10">
              <div className="text-center">
                ...
                <p className="font-black text-[15px] tracking-[0.1em]">{income.operation_number}</p>
              </div>
              <div className="text-center border-s border-[#ffffff1a]">
                ...
                <p className="font-black text-[15px]">{income.income_date ? format(new Date(income.income_date), 'yyyy/MM/dd') : '-'}</p>
              </div>
            </div>
            
            <div className="absolute -top-24 -right-24 w-64 h-64 bg-teal-400/10 rounded-full blur-3xl" />
            <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-cyan-400/10 rounded-full blur-3xl" />
          </div>
          ...
          <div className="p-8 md:p-12 space-y-10 flex-grow flex flex-col bg-white print-body">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 print-info-grid">
              <div className="relative group">
                <div className="absolute inset-0 bg-cyan-50 rounded-3xl -rotate-1 group-hover:rotate-0 transition-transform duration-300" />
                <div className="relative rounded-3xl p-6 border border-cyan-100 bg-white shadow-sm flex flex-col h-full">
                  <div className="flex items-center gap-3 mb-5 pb-3 border-b border-gray-50">
                    <div className="h-10 w-10 rounded-xl bg-cyan-500 flex items-center justify-center text-white shadow-md">
                      <Building2 size={20} />
                    </div>
                    <div>
              ...
              <div className="relative group">
                ...
                <div className="relative rounded-3xl p-6 border border-teal-100 bg-white shadow-sm flex flex-col h-full">
                  <div className="flex items-center gap-3 mb-5 pb-3 border-b border-gray-50">
                    ...
                    <div>
                      <h3 className="font-black text-gray-900 text-sm">{isRtl ? "بيانات الإيراد" : "Income Details"}</h3>
                      <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Income Info</p>
                    </div>
                  </div>
                  <div className="space-y-3.5 flex-1">
                    <DetailItem label={isRtl ? "نوع الإيراد" : "Income Type"} value={income.income_type} isRtl={isRtl} />
                    <DetailItem label={isRtl ? "طريقة الدفع" : "Payment Method"} value={income.payment_method} isRtl={isRtl} />
                    <DetailItem label={isRtl ? "تاريخ الإيراد" : "Income Date"} value={income.income_date} isRtl={isRtl} />
                    <DetailItem label={isRtl ? "بواسطة" : "Created By"} value={income.created_by || '-'} isRtl={isRtl} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-100 overflow-hidden shadow-sm bg-gray-50 p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-black text-gray-900 text-sm">{isRtl ? "البيان / الوصف" : "Description"}</h3>
              </div>
              <p className="text-gray-700 font-medium leading-relaxed">
                {income.description || (isRtl ? "لا يوجد بيان" : "No description")}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-end">
              <div className="space-y-6">
                <div className="flex items-center gap-6 p-6 rounded-3xl border-2 border-dashed border-gray-100 bg-gray-50/30 print-qr-section">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-1.5 text-cyan-600 text-[10px] font-bold">
                      <CheckCircle size={12} />
              ...
              <div className="relative">
                <div className="absolute inset-0 bg-cyan-600 rounded-3xl translate-x-1.5 translate-y-1.5 opacity-5" />
                <div className="relative bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden print-totals">
                  <div className="p-6 md:p-8 space-y-4">
                    <div className="flex justify-between items-center text-gray-500 font-bold text-sm print-totals-row">
                      ...
                      <span>{Number(income.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })} {currency}</span>
                    </div>
                    <div className="flex justify-between items-center text-amber-600 font-bold text-sm">
                      ...
                      <span>{Number(income.vat || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })} {currency}</span>
                    </div>
                    <div className="pt-4 border-t border-gray-100 mt-2">
                      <div className="flex justify-between items-center p-5 bg-cyan-600 rounded-2xl text-white shadow-lg shadow-cyan-200 print-grand-total">
                        ...
                        <div className="text-right">
                          <span className="block text-[10px] text-teal-200 font-black mb-1 uppercase tracking-widest">Grand Total</span>
                          <span className="text-2xl md:text-3xl font-black tabular-nums">
                            {Number(income.total).toLocaleString('en-US', { minimumFractionDigits: 2 })}
            ...
            <div className="grid grid-cols-2 gap-12 pt-12 mt-auto border-t border-gray-50 print-stamps">
              <div className="text-center group">
                ...
                <div className="w-36 h-36 mx-auto bg-white rounded-3xl border-2 border-dashed border-gray-100 flex items-center justify-center p-6 shadow-inner group-hover:border-cyan-200 transition-colors">
                  {company.stamp_path ? (
                    <img 
  ...
  return (
    <div className={cn("flex flex-col gap-1", isRtl ? "text-right" : "text-left")}>
      ...
      <p className={cn(
        ...
        accent ? "text-cyan-600" : "text-gray-900"
      )}>
