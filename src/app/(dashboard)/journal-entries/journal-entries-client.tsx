...
function JournalEntriesContent() {
  ...
  const companyId = "1"; // In a real app, this comes from session/context
  ...
  const removeRow = (index: number) => {
    if (lines.length <= 1) return toast.error("يجب وجود سطر واحد على الأقل");
  ...
  const handleLineChange = (index: number, field: keyof JournalLine, value: string) => {
    ...
    // Auto zero other side if one side is filled (optional professional touch)
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (totals.diff > 0.01) return toast.error("يجب أن يتساوى المدين والدائن");
    
    const validLines = lines.filter(l => (parseFloat(l.debit) > 0 || parseFloat(l.credit) > 0) && l.account_id);
    if (validLines.length < 2) return toast.error("يجب إدخال سطرين صحيحين على الأقل");
    ...
    try {
      const res = await fetch("/api/journal-entries/save", {
        ...
        body: JSON.stringify({
          ...
          created_by: "المدير", // Should be from session
          is_edit: isEdit
        })
      });
      const data = await res.json();
      ...
      toast.success(isEdit ? "تم تحديث القيد بنجاح" : "تم حفظ القيد بنجاح");
      resetForm();
      fetchMetadata();
    } catch (error: any) {
  ...
  const resetForm = () => {
    ...
    setIsEdit(false);
    fetchMetadata(); // To get new entry number
  };
  ...
  return (
    <div className="max-w-[1600px] mx-auto p-4 space-y-8 animate-in fade-in duration-700" dir="rtl">
      {/* Header Section */}
      ...
      {/* Main Form Card */}
      <div className="bg-white/90 backdrop-blur-xl rounded-[25px] shadow-xl border border-white/20 overflow-hidden transition-all hover:shadow-2xl">
        <div className="bg-gradient-to-r from-[#2c3e50] to-[#3498db] p-6 text-white flex justify-between items-center">
          <h3 className="text-xl font-bold flex items-center gap-3">
            <PlusCircle className="w-6 h-6" />
            {isEdit ? "تعديل القيد" : "إنشاء قيد جديد"}
          </h3>
            ...
            <div className="border-2 border-blue-50 rounded-2xl overflow-hidden shadow-sm">
              <table className="w-full border-collapse">
                <thead className="bg-gradient-to-r from-gray-50 to-blue-50 text-gray-700 font-bold border-b-2 border-blue-100">
                  <tr>
                    <th className="p-4 text-right w-1/4">الحساب</th>
                    <th className="p-4 text-right w-1/4">الوصف</th>
                    <th className="p-4 text-center w-1/6">مدين</th>
                    <th className="p-4 text-center w-1/6">دائن</th>
                ...
                <tfoot className="bg-gray-50/50">
                  <tr className="font-bold text-lg">
                    <td colSpan={2} className="p-6 text-left text-gray-600">المجموع الكلي:</td>
            ...
            <div className="flex flex-col gap-4">
              {totals.diff > 0.01 ? (
                <div className="flex items-center gap-3 p-5 bg-red-50 border-2 border-red-200 rounded-2xl text-red-700 animate-pulse">
                  ...
                  <div>
                    <p className="font-bold text-lg">القيد غير متوازن!</p>
                    <p className="text-sm font-medium opacity-80">الفرق الحالي: {totals.diff.toFixed(2)} ريال</p>
                  </div>
                </div>
              ) : totals.debit > 0 ? (
                <div className="flex items-center gap-3 p-5 bg-green-50 border-2 border-green-200 rounded-2xl text-green-700">
                  ...
                  <div>
                    <p className="font-bold text-lg">القيد متوازن وجاهز للحفظ</p>
                    <p className="text-sm font-medium opacity-80">جميع الحسابات مطابقة بنجاح</p>
                  </div>
                </div>
              ) : null}
            </div>
            ...
            <div className="flex justify-between items-center gap-4 pt-6 border-t border-gray-100">
              ...
              <div className="flex gap-4">
                ...
                >
                  ...
                  {isEdit ? "حفظ التعديلات" : "ترحيل القيد"}
                </button>
              </div>
            </div>
          </form>
      ...
      <div className="bg-white/90 backdrop-blur-xl rounded-[25px] shadow-xl border border-white/20 overflow-hidden">
        <div className="bg-gradient-to-r from-[#2c3e50] to-[#2ecc71] p-8 text-white flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-extrabold">سجل القيود اليومية</h3>
              <p className="text-white/70 font-medium">مراجعة وإدارة جميع العمليات المالية المرحلة</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {Object.keys(filteredGroups).length > 0 ? (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-6">
                      <div className="flex flex-col items-end">
                        <span className="text-xs text-gray-400 font-bold uppercase">الإجمالي</span>
                      ...
                      >
                        <div className="p-6 bg-white border-t border-gray-100">
                          <table className="w-full">
                            <thead>
                              <tr className="text-gray-400 text-sm font-bold border-b border-gray-50">
                                <th className="pb-3 text-right">الحساب</th>
                          ...
                          <div className="mt-4 pt-4 border-t border-gray-50 flex justify-between text-xs font-bold text-gray-400">
                            <span>أنشئ بواسطة: {lines[0].created_by}</span>
                            <span>رقم العملية: {lines[0].id}</span>
                          </div>
                        </div>
                      </motion.div>
                    )}
              ...
              <div className="text-center py-20 bg-gray-50/50 rounded-3xl border-2 border-dashed border-gray-200">
                ...
                <h4 className="text-xl font-bold text-gray-400">لا توجد قيود مسجلة حالياً</h4>
                <p className="text-gray-300">ابدأ بإضافة أول قيد يومية من النموذج أعلاه</p>
              </div>
            )}
          </div>
        </div>
