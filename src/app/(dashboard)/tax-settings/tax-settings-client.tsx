  ...
  return (
    <div className="p-6 space-y-8 max-w-[1600px] mx-auto">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpened}>
        <DialogContent className="bg-[#0d121f] border-white/10 text-white max-w-2xl rounded-3xl overflow-hidden p-0">
          ...
          <form onSubmit={handleAddUpdateTax}>
            <DialogHeader className="p-8 border-b border-white/5">
              <DialogTitle className="text-2xl font-black">{editingTax ? t("editTaxType") : t("addTaxType")}</DialogTitle>
              <DialogDescription className="text-white/40 font-medium">
                أدخل تفاصيل نوع الضريبة الجديد وكيفية تطبيقه في النظام
              </DialogDescription>
            </DialogHeader>
            
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6 relative">
              <div className="space-y-2">
                <Label className="text-white/60 font-black text-xs uppercase tracking-wider">{t("taxCode")}</Label>
                <Input 
                  name="tax_code" 
                  defaultValue={editingTax?.tax_code}
                  placeholder="مثال: VAT_15" 
                  required
                  className="bg-white/5 border-white/10 text-white rounded-xl focus:ring-blue-500/20 h-12"
                />
              </div>
              <div className="space-y-2">
                <Label className="text-white/60 font-black text-xs uppercase tracking-wider">{t("taxRate")}</Label>
                <div className="relative">
                  <Input 
                    name="tax_rate" 
                    type="number"
                    step="0.01"
                    defaultValue={editingTax?.tax_rate}
                    placeholder="15.00" 
                    required
                    className="bg-white/5 border-white/10 text-white rounded-xl focus:ring-blue-500/20 h-12 pe-10"
                  />
                  <Percent className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/30" />
                </div>
              </div>
              <div className="space-y-2">
                <Label className="text-white/60 font-black text-xs uppercase tracking-wider">{t("taxName")} (عربي)</Label>
                <Input 
                  name="name_ar" 
                  defaultValue={editingTax?.name_ar}
              ...
              <div className="space-y-2">
                <Label className="text-white/60 font-black text-xs uppercase tracking-wider">{t("taxName")} (English)</Label>
                <Input 
                  name="name_en" 
                  defaultValue={editingTax?.name_en}
                  placeholder="Value Added Tax" 
                  className="bg-white/5 border-white/10 text-white rounded-xl focus:ring-blue-500/20 h-12"
                />
              </div>
              <div className="space-y-2">
                <Label className="text-white/60 font-black text-xs uppercase tracking-wider">{t("applyTo")}</Label>
              ...
              <div className="space-y-2">
                <Label className="text-white/60 font-black text-xs uppercase tracking-wider">{t("status")}</Label>
              ...
              <div className="md:col-span-2 space-y-2">
                <Label className="text-white/60 font-black text-xs uppercase tracking-wider">{t("notes")}</Label>
                <Input 
                  name="description" 
                  defaultValue={editingTax?.description}
                  placeholder="وصف إضافي للضريبة..." 
                  className="bg-white/5 border-white/10 text-white rounded-xl focus:ring-blue-500/20 h-12"
                />
              </div>
              <div className="md:col-span-2 flex items-center gap-3 p-4 bg-white/5 rounded-2xl border border-white/10">
                <Switch name="is_default" defaultChecked={editingTax?.is_default} />
                <Label className="text-white font-bold">{t("default")}</Label>
              </div>
            </div>
            ...
            <DialogFooter className="p-8 border-t border-white/5 bg-white/[0.02]">
              <Button 
                ...
                className="text-white/40 hover:text-white hover:bg-white/10 px-8 rounded-xl font-bold"
              >
                {t("cancel")}
              </Button>
              <Button 
                type="submit" 
                className="bg-blue-600 hover:bg-blue-500 text-white font-black px-12 h-12 rounded-xl shadow-lg shadow-blue-500/20"
              >
