...
export async function GET(request: NextRequest) {
  try {
    ...
    if (manualIncomeError) {
      console.error("Manual income error:", manualIncomeError);
    }
    ...
    let expensesTotal = 0;
    for (const exp of expensesResult) {
    ...
    const totalIncome = invoiceIncomeValue + manualIncomeTotal + receiptVouchersTotal;
    const totalExpenses = expensesTotal + payrollsTotal;
