...
export async function GET(request: NextRequest) {
  try {
    ...
    const quotations = await query<Quotation>(
      `SELECT q.*, c.customer_name as client_name, c.vat_number as client_vat
       FROM quotations q
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!company_id || !client_id || !quotation_number) {
    ...
    const result = await execute(
      `INSERT INTO quotations (
        company_id, quotation_number, client_id, client_name, client_vat, client_address,
