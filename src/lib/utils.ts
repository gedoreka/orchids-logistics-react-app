...
export function getPublicUrl(path: string | null | undefined, bucket: string = 'expenses'): string | null {
  ...
  // For expenses, we usually use Supabase for new files (high timestamp)
  ...
  if (isSupabase) {
    ...
    // CRITICAL: Encode each segment individually to handle Arabic/Spaces correctly
  ...
  // 5. Fallback to Hostinger for old/relative paths
