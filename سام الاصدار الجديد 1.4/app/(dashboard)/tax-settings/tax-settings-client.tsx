    ...
    try {
      const res = await fetch("/api/taxes/settings", {
        ...
        if (!res.ok) {
          ...
          console.error("Save settings HTTP error:", res.status, text);
          try {
            const errData = JSON.parse(text);
            showNotification("error", "خطأ", errData.error || `HTTP ${res.status}`);
          } catch {
            showNotification("error", "خطأ", `HTTP ${res.status}: ${text.slice(0, 100)}`);
          }
          return;
        }
        const data = await res.json();
      if (data.success) {
        ...
        showNotification("error", "خطأ", data.error || "فشل في حفظ الإعدادات");
      }
    } catch (err: any) {
      console.error("Save settings error:", err);
      showNotification("error", "خطأ", err?.message || "حدث خطأ أثناء حفظ الإعدادات");
    } finally {
      setSaving(false);
    }
  };
