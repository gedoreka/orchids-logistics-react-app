...
import {
  Edit, Building2, User, Mail, Phone, MapPin, FileText, Receipt,
  Hash, Globe, Building, MapPinned, ArrowRight, Wallet, Calculator,
  Save, Loader2, Power, Eye, Route, LayoutDashboard, Users
} from "lucide-react";
...
export function EditCustomerClient({ customer, accounts, costCenters, companyId }: EditCustomerClientProps) {
  ...
  const handleSelectChange = (name: string, value: string | number) => {
    setFormData(prev => {
      ...
      if (name === "country") { newData.region = ""; newData.city = ""; newData.district = ""; }
      else if (name === "region") { newData.city = ""; newData.district = ""; }
      else if (name === "city") { newData.district = ""; }
      return newData;
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => { router.push(`/customers/${customer.id}`); router.refresh(); }, 2000);
      } else {
  ...
  return (
    <div className="w-full px-2 pt-4 pb-6">
      ...
      >
        {/* Rainbow bar */}
        <div className="h-1.5 w-full bg-gradient-to-r from-amber-500 via-orange-500 via-blue-500 via-indigo-500 to-amber-500" />
        ...
        <div className="p-5 md:p-8 space-y-5">
          ...
          >
            <div className="flex items-center gap-4">
              <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center shadow-md shadow-amber-500/25 flex-shrink-0">
                <Edit size={22} strokeWidth={2.5} className="text-white" />
              </div>
              <div>
                <div className="flex items-center gap-1.5 text-[11px] font-bold text-slate-400 mb-1">
                  ...
                  <ArrowRight size={11} className="text-slate-500 rotate-180" />
                  <span className="text-amber-400">تعديل</span>
                </div>
                <h1 className="text-xl font-black text-white tracking-tight">
                  {customer.company_name || customer.customer_name}
                </h1>
                <p className="text-white/40 text-xs font-semibold mt-0.5">
                  {[formData.city, formData.district && `حي ${formData.district}`, formData.street_name && `شارع ${formData.street_name}`].filter(Boolean).join(' ') || 'تحديث معلومات الحساب'}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Link href={`/customers/${customer.id}`}>
                <motion.button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-blue-500/10 text-blue-400 font-bold text-sm hover:bg-blue-500/20 transition-all border border-blue-500/20"
                >
                  <Eye size={15} />
                  <span>عرض</span>
                </motion.button>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
              ...
              >
                ...
                <div className="p-5 space-y-4">
                  <Field icon={<Mail size={14} />} label="البريد الإلكتروني" name="email" type="email" value={formData.email} onChange={handleChange} placeholder="example@domain.com" />
            ...
            >
              ...
              <div className="p-5 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-7 gap-4">
                ...
                <Field icon={<MapPin size={14} />} label="العنوان المختصر" name="short_address" value={formData.short_address} onChange={handleChange} placeholder="RREE1234" />
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <motion.button
                  type="submit"
                  disabled={loading}
                  whileHover={{ scale: loading ? 1 : 1.04, boxShadow: "0 8px 24px rgba(245,158,11,0.35)" }}
                  whileTap={{ scale: 0.96 }}
                  className="flex items-center gap-2.5 px-7 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 text-white font-black text-sm shadow-lg shadow-amber-500/25 hover:from-amber-400 hover:to-orange-500 transition-all disabled:opacity-50"
                >
...
function Field({ icon, label, name, value, onChange, placeholder, required, type = "text" }: {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <input
        ...
        className="w-full px-4 py-2.5 rounded-xl border border-white/10 bg-white/5 focus:bg-white/10 focus:border-amber-500/50 focus:ring-2 focus:ring-amber-500/10 outline-none transition-all text-sm font-bold text-white placeholder:text-white/20 placeholder:font-medium"
      />
    </div>
  );
}
