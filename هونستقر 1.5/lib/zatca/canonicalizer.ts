...
export function removeSignatureTag(xml: string): string {
  // Remove cac:Signature element (the one inside Invoice, NOT the ds:Signature in extensions)
  return xml.replace(/<cac:Signature>[\s\S]*?<\/cac:Signature>/g, "");
}

export function removeSignatureAndQR(xml: string): string {
  let result = removeSignatureElement(xml); // removes ext:UBLExtensions
  result = removeSignatureTag(result);       // removes cac:Signature
  result = removeQRElement(result);          // removes QR AdditionalDocumentReference
  return result;
}
